...
public extension OverlayGraph {
    ...
    func xorShapes() -> [FixShape] {
        var visited = self.filter(operation: .xor)
    ...
    private func getContour(index: Int, visited: inout [Bool]) -> Contour {
        ...
        let isSubject = link.fill & .bothTop == .subjectTop || link.fill & .bothBottom == .subjectBottom
        
        let topFill: Int
        let bottomFill: Int
        if isSubject {
            topFill = .subjectTop
            bottomFill = .subjectBottom
        } else {
            topFill = .clipTop
            bottomFill = .clipBottom
        }
        ...
        repeat {
            ...
            if node.count == 2 {
                ...
                let isFillTop = link.fill & .bothTop == topFill
        ...
        let isFillBottom = leftLink.fill & .bothBottom == bottomFill
