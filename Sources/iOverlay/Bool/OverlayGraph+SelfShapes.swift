...
public extension OverlayGraph {
    ...
    func shapes(type: ShapeType) -> [FixShape] {
        var visited = self.filter(operation: type == .subject ? .subject : .clip)
        ...
        for i in 0..<links.count {
            if !visited[i] {
                let contour = self.getContour(index: i, type: type, visited: &visited)
    ...
    private func getContour(index: Int, type: ShapeType, visited: inout [Bool]) -> Contour {
        let top = type == ShapeType.subject ? SegmentFill.subjectTop : SegmentFill.clipTop
        let bottom = type == ShapeType.subject ? SegmentFill.subjectBottom : SegmentFill.clipBottom
        ...
        repeat {
            ...
            if node.count == 2 {
                ...
                let isFillTop = link.fill & top == top
        ...
        let isFillBottom = leftLink.fill & bottom == bottom
